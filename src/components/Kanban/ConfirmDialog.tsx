...
export default function ConfirmDialog({ isOpen, title, message, onConfirm, onCancel }: ConfirmDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-neutral-900 border border-neutral-800 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden p-6 space-y-4">
        <h3 className="text-lg font-bold text-white flex items-center gap-2">
          <AlertCircle className="text-red-500" size={20} /> {title}
        </h3>
        ...
        <div className="flex gap-3 justify-end pt-3 border-t border-neutral-800/80">
          ...
          <button 
            onClick={onConfirm} 
            className="px-4 py-2.5 rounded-xl text-xs font-medium bg-red-600/90 hover:bg-red-500 text-white transition-colors shadow-lg shadow-red-600/20"
          >
            Sí, eliminar
          </button>
        </div>
